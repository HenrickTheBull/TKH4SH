...
def enable_logging(chat_id, chat_name):
    with INSERTION_LOCK:
        curr = SESSION.query(LoggerSettings).get(str(chat_id))
        if not curr:
            curr = LoggerSettings(str(chat_id), chat_name)
            SESSION.add(curr)
            SESSION.commit()
...
def is_logging_enabled(chat_id):
    try:
        return SESSION.query(LoggerSettings).get(str(chat_id)) is not None
    finally:
        SESSION.close()
